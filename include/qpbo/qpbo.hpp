...
#include <assert.h>
#define QPBO_USER_ASSSERT assert // used for checking user input
#define QPBO_CODE_ASSERT assert // used for checking algorithm's correctness
...
private:
    ...
    Node* GetMate0(Node* i) { QPBO_CODE_ASSERT(i< nodes[1]); return (Node*)((char*)i + node_shift); }
    Node* GetMate1(Node* i) { QPBO_CODE_ASSERT(i>=nodes[1]); return (Node*)((char*)i - node_shift); }
    Node* GetMate(Node* i) { return IsNode0(i) ? GetMate0(i) : GetMate1(i); }
    bool IsArc0(Arc* a) { return (a<arcs[1]); }
    Arc* GetMate0(Arc* a) { QPBO_CODE_ASSERT(a< arcs[1]); return (Arc*)((char*)a + arc_shift); }
    Arc* GetMate1(Arc* a) { QPBO_CODE_ASSERT(a>=arcs[1]); return (Arc*)((char*)a - arc_shift); }
...
{
    QPBO_USER_ASSSERT(i >= 0 && i < node_num);
...
{
    QPBO_CODE_ASSERT(i >= nodes[0] && i<node_last[0]);
...
{
    QPBO_USER_ASSSERT(e >= 0 && arcs[0][2*e].sister);
...
{
    QPBO_CODE_ASSERT(node_num_max_new > node_shift/((int)sizeof(Node)));
    ...
    void QPBO<REAL>::reallocate_arcs(int arc_num_max_new)
{
    ...
    QPBO_CODE_ASSERT(arc_num_max > arc_num_max_old);
...
{
    QPBO_USER_ASSSERT(_i >= 0 && _i < node_num);
    QPBO_USER_ASSSERT(_j >= 0 && _j < node_num);
    QPBO_USER_ASSSERT(_i != _j);
...
{
    QPBO_USER_ASSSERT(e >= 0 && arcs[0][2*e].sister);
    QPBO_USER_ASSSERT(arcs[0][2*e].head==&nodes[0][_i] || arcs[0][2*e].head==&nodes[1][_i] || arcs[0][2*e].head==&nodes[0][_j] || arcs[0][2*e].head==&nodes[1][_j]);
    QPBO_USER_ASSSERT(arcs[0][2*e+1].head==&nodes[0][_i] || arcs[0][2*e+1].head==&nodes[1][_i] || arcs[0][2*e+1].head==&nodes[0][_j] || arcs[0][2*e+1].head==&nodes[1][_j]);
    QPBO_USER_ASSSERT(_i != _j);
    ...
    if (stage == 0)
    {
        ...
        QPBO_CODE_ASSERT(a->sister==a_rev && a->sister==a_rev);

        Node* i = a_rev->head;
        Node* j = a->head;
        QPBO_CODE_ASSERT(IsNode0(i));
    ...
    else
    {
        ...
        QPBO_CODE_ASSERT(a[0]->sister==a_rev[0] && a[1]->sister==a_rev[1] && a[0]==a_rev[0]->sister && a[1]==a_rev[1]->sister);
    ...
    void QPBO<REAL>::TransformToSecondStage(bool copy_trees)
{
    ...
    if (!copy_trees)
    {
        ...
        for (a[0]=arcs[0], a[1]=arcs[1]; a[0]<arc_max[0]; a[0]+=2, a[1]+=2)
        {
            ...
            QPBO_CODE_ASSERT(IsNode0(a[0]->sister->head));
...
{
    QPBO_CODE_ASSERT(IsNode0(i) && IsNode0(j) && swap>=0 && swap<=1);
...
{
    QPBO_CODE_ASSERT(a1->sister->head == a2->sister->head && IsNode0(a1->sister->head));
    QPBO_CODE_ASSERT(a1->head == a2->head || a1->head == GetMate(a2->head));
    ...
    {
        QPBO_CODE_ASSERT(a1->head == GetMate(a2->head));
        ...
        QPBO_CODE_ASSERT (_a1[0]->r_cap >= 0);
...
{
    QPBO_CODE_ASSERT(first_free && IsNode0(i) && IsNode0(j) && i!=j);
    ...
    void QPBO<REAL>::AllocateNewEnergy(int* mapping)
{
    ...
    ////////////////////////////////////////////////////////////////
    QPBO_CODE_ASSERT(nodes[0] + nodeNumNew <= node_max[0]);
    // Reset:
    node_last[0] = nodes[0];
    node_last[1] = nodes[1];
    node_num = 0;
    ...
    for (e=0; e<edgeNumOld; e++)
    {
        if ( arcs[0][2*e].sister )
        {
            ...
            QPBO_CODE_ASSERT(i_index > 0 && i_index < nodeNumNew);

            first_free = &arcs[0][2*edgeNumNew++];
            if (IsNode0(a->head))
            {
                j_index = mapping[(int)(a->head - nodes[0])] / 2;
                QPBO_CODE_ASSERT(j_index > 0 && j_index < nodeNumNew);
...
struct List // contains LIST_NUM lists containing integers 0,1,...,num-1. In the beginning, each integer is in list 1.
{
    ...
    {
        QPBO_CODE_ASSERT (r_to>=1 && r_to<=LIST_NUM);
        Remove(i);
        next[i] = -r_to;
        prev[i] = prev[-r_to];
        next[prev[-r_to]] = i;
    ...
    {
        QPBO_CODE_ASSERT (r_from>=1 && r_from<=LIST_NUM && r_to>=1 && r_to<=LIST_NUM && r_from!=r_to);
    ...
    bool QPBO<REAL>::Probe(int* mapping)
{
    ...
    while ( 1 )
    {
        ...
        for (i_index=list.GetNext(-current_list) ; i_index>=0; i_index=i_index_next)
        {
            ...
            i = nodes[0]+i_index;
            QPBO_CODE_ASSERT (!i->is_removed && i->label<0);
            ...
            {
                j = *ptr;
                QPBO_CODE_ASSERT(!j->is_removed && j->label<0);
            ...
            if (probe_options.dilation >= 0)
            {
                    ...
                    if (probe_options.dilation >= 1)
                    {
                        for (a=i->first; a; a=a->next)
                        {
                            ...
                            if (!(j->list_flag & MASK_NEXT))
                            {
                                ...
                                {
                                    j_index = (int)(j-nodes[0]);
                                    QPBO_CODE_ASSERT(j_index != i_index_next);
                                    list.Move(j_index, 3);
                                }
                            }
                        }
                ...
                QPBO_CODE_ASSERT (i->list_flag & MASK_CURRENT);
                i->list_flag &= ~MASK_CURRENT;
        ...
        QPBO_CODE_ASSERT(list.GetNext(-1) == -1);
        list.MoveList(4, 2);
        if (list.GetNext(-3) < 0)
        {
            // list 3 is empty
    ...
    bool QPBO<REAL>::Improve(int N, int* order_array, int* fixed_nodes)
{
    ...
    for (p=0; p<N; p++)
    {
        ...
        QPBO_CODE_ASSERT(i_index>=0 && i_index<GetNodeNum());
...
#define QPBO_INFINITE_D ((int)(((unsigned)-1)/2))        /* infinite distance to the terminal */
    ...
    if (a0->sister->r_cap)
    {
        ...
        if (!j->is_sink && (a=j->parent))
        {
            ...
            while ( 1 )
            {
                ...
                if (a==QPBO_MAXFLOW_ORPHAN) { d = QPBO_INFINITE_D; break; }
                j = a -> head;
            }
            if (d<QPBO_INFINITE_D) /* j originates from the source - done */
            {
                if (d<d_min)
                {
                    a0_min = a0;
    ...
    void QPBO<REAL>::process_sink_orphan(Node *i)
{
    ...
    if (a0->r_cap)
    {
        ...
        if ((a=j->parent) && j->is_sink)
        {
            ...
            if (d<QPBO_INFINITE_D) /* j originates from the sink - done */
            {
                if (d<d_min)
                {
                    a0_min = a0;
    ...
    void QPBO<REAL>::test_consistency(Node* current_node)
{
    ...
    for (r=0; r<3; r++)
    {
        ...
        for ( ; ; i=i->next)
        {
            QPBO_CODE_ASSERT((IsNode0(i) && !i->is_removed) || (!IsNode0(i) && !GetMate1(i)->is_removed));
            num2 ++;
            if (i->next == i)
            {
                if (r<2) QPBO_CODE_ASSERT(i == queue_last[r]);
                else     QPBO_CODE_ASSERT(i == current_node);
                break;
            }
        }
    }
    QPBO_CODE_ASSERT(num1 == num2);
    ...
    for (i=nodes[0]; i<node_last[stage]; i++)
    {
        ...
        {
            if (!i->is_sink) QPBO_CODE_ASSERT(i->tr_cap > 0);
            else             QPBO_CODE_ASSERT(i->tr_cap < 0);
        }
        else
        {
            if (!i->is_sink) QPBO_CODE_ASSERT (i->parent->sister->r_cap > 0);
            else             QPBO_CODE_ASSERT (i->parent->r_cap > 0);
        }
        ...
        if (i->parent && !i->next)
        {
            if (!i->is_sink)
            {
                QPBO_CODE_ASSERT(i->tr_cap >= 0);
                for (a=i->first; a; a=a->next)
                {
                    if (a->r_cap > 0) QPBO_CODE_ASSERT(a->head->parent && !a->head->is_sink);
                }
            }
            else
            {
                ...
                for (a=i->first; a; a=a->next)
                {
                    if (a->sister->r_cap > 0) QPBO_CODE_ASSERT(a->head->parent && a->head->is_sink);
                }
            }
        }
        ...
        {
            QPBO_CODE_ASSERT(i->TS <= i->parent->head->TS);
            if (i->TS == i->parent->head->TS) QPBO_CODE_ASSERT(i->DIST > i->parent->head->DIST);
        }
    }
}
    ...
    void QPBO<REAL>::ComputeWeakPersistencies()
{
    ...
    for (i=nodes[0]; i<node_last[0]; i++)
    {
        QPBO_CODE_ASSERT(i->label>=-1 && i->label<=1);
